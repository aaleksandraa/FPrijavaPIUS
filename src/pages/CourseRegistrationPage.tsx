            ...
            <div className="bg-gray-800 rounded-lg p-4 max-h-60 overflow-y-auto text-sm text-gray-300 whitespace-pre-wrap">
              {(() => {
                ...
                return template
                  ...
                  .replace(/{drzava}/g, formData.country)
                  .replace(/{brojLicnogDokumenta}/g, formData.id_document_number)
                  .replace(/{nazivFirme}/g, formData.company_name || '')
                  .replace(/{pdvBroj}/g, formData.vat_number || '')
                  .replace(/{adresaFirme}/g, formData.company_address || '')
                  .replace(/{postanskiBrojFirme}/g, formData.company_postal_code || '')
                  .replace(/{mjestoFirme}/g, formData.company_city || '')
                  .replace(/{drzavaFirme}/g, formData.company_country || '')
                  .replace(/{registracijaFirme}/g, formData.company_registration || '')
                  .replace(/{cijena}/g, `€${Number(pkg.price).toFixed(2)}`)
                  .replace(/{datum}/g, new Date().toLocaleDateString('de-AT'));
              })()}
            </div>
